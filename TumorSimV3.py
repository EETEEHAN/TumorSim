        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if cell.cell_type == 'cancer':
                    if cell.subtype:
                        grid_data[y][x] = SUBTYPE_COLORS.get(cell.subtype[0], 6)  # fallback to 'Unclassified' index if needed
                else:
                    grid_data[y][x] = 0  # normal = white

        plt.figure(figsize=(6, 6))
        plt.imshow(grid_data, cmap=custom_cmap, interpolation='nearest')
        colorbar_labels = ['Normal'] + list(SUBTYPE_COLORS.keys())
        cbar = plt.colorbar(ticks=range(len(colorbar_labels)))
        cbar.ax.set_yticklabels(colorbar_labels)
        plt.title('Tumor Growth by Primary Subtype')
        plt.show()
